...
//		StartCoroutine ("runEasyAI"); //AI vs AI
    ...
    public bool inCheck(Point p)
    {
		...
		if(((Piece)boardPieces[m+2,n+1].GetComponent("Piece")).getType() == Piece.PieceTypeE.KNIGHT){
			return true;
		}
		if(((Piece)boardPieces[m+2,n-1].GetComponent("Piece")).getType() == Piece.PieceTypeE.KNIGHT){
			return true;
		...
		if(((Piece)boardPieces[m+1,n+2].GetComponent("Piece")).getType() == Piece.PieceTypeE.KNIGHT){
			return true;
		}
		if(((Piece)boardPieces[m+1,n-2].GetComponent("Piece")).getType() == Piece.PieceTypeE.KNIGHT){
			return true;
